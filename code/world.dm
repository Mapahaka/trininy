/world
	mob = /mob/new_player
	turf = /turf/simulated/floor/plating/airless/asteroid
	area = /area
	view = "15x15"
...
/world/Topic(T, addr, master, key)
	...
	if (T == "ping")
		...
		F["s_x"] >> saved_x; F["s_y"] >> saved_y; F["s_z"] >> saved_z
...
//	s += "[game_version]"
	...
	*/
	...
	if (src.status != s)
		src.status = s
/*
/proc/start_temperature_change()
	var/change = 0.06
	spawn while(1)
		set background = 1
		for (var/area/A in world)
			if (A.type == world.area)
			for (var/turf/T in A)
				if (T.temperature >= 279.15) change=-0.06
				if (T.temperature <= 218.15) change=0.06
				T.temperature += change
		sleep(100)
start_temperature_change()
*/
