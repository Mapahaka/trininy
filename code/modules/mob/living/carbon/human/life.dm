...
/mob/living/carbon/human
	...
	proc/handle_mutations_and_radiation()
		...
		if ((TNIGGA in mutations) && istype(src.back, /obj/item/weapon/storage/backpack))
			var/obj/item/weapon/storage/backpack/B = src.back
			for(var/obj/item/W in range(1, src))
				if(prob(1) && W.w_class<4 && istype(W.loc,/turf) && (B.contents.len <= B.storage_slots))
					W.loc=src.back
	...
	proc/handle_breath(datum/gas_mixture/breath)
		...
		if( (abs(310.15 - breath.temperature) > 50) && !(COLD_RESISTANCE in mutations) && !(wear_suit)) // Hot air hurts :(
